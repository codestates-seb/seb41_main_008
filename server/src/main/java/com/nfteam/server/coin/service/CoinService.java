...
public class CoinService {
    ...
    public CoinPurchaseReadyResponse startPayment(CoinPurchaseRequest request, MemberDetails memberDetails) {
        ...
        Double doubleCount = coinCount * 1000;
        Integer intCount = doubleCount.intValue();
        Integer intTotal = totalPrice.intValue();
        ...
        parameters.add("quantity", intCount); // 상품수량
        parameters.add("total_amount", intTotal); // 상품 총액
        ...
        if (coinPurchaseReadyResponse != null) {
            // 현재 해당 회원이 가진 해당 코인이 없을 경우 신규 관계 생성
            CoinMemberRel coinMemberRel = coinMemberRelRepository.findByMemberAndCoin(buyer, coin)
                    .orElseGet(() -> coinMemberRelRepository.save(new CoinMemberRel(coin, buyer)));
            coinMemberRel.addCoinCount(coinCount);
            // 코인 주문 tid 세팅
    ...
    public CoinPurchaseApproveResponse approvePayment(String pgToken, String tid) {
        CoinOrder coinOrder = coinOrderRepository.findByTid(tid)
        ...
        if (coinPurchaseApproveResponse != null) {
            ...
            log.info("승인 및 결제완료ㅇㅇㅇ {}", coinPurchaseApproveResponse.getTid());
