...
public class JwtVerificationFilter extends OncePerRequestFilter {
    ...
    private boolean isLoginLogoutRequest(HttpServletRequest request) {
        boolean isHomeLoginRequest = request.getRequestURI().contains("/auth/login");
        boolean isGoogleLoginRequest = request.getRequestURI().contains("/oauth2/authorization/google");
        boolean isLogoutRequest = request.getRequestURI().contains("/auth/logout");
        return isHomeLoginRequest || isGoogleLoginRequest || isLogoutRequest;
    }
