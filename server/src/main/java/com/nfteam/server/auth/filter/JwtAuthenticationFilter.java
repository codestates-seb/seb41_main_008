...
/**
 * 로그인 시 인증정보 확인(AuthenticationManager가 UserDetailService를 호출하여 UserDetail을 확인) 후 토큰
 * 발급(jwtTokenizer)
 */
...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    /**
     * 입력받은 username, password를 바탕으로 인증정보를 생성하는 로직 1. ObjectMapper로 request에서 loginDto로
     * username,password를 담는다. 2. 이 정보를 가지고 '인증 전'(권한 설정이 되어 있지 않은) authenticationToken 발급 3.
     * authenticationToken을 이용하여 UserDetailsService에서 인증 정보를 받아온다. -> (내부적으로 해주기에 코드상에 보이진 않음)
     *
    ...
    /**
     * attempt 성공시 진입하게 될 메서드. 1. 인증된 Authentication의 field에서 Member정보를 가져온다. 2. 가져온 member를 이용하여
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authentication) throws ServletException, IOException {
        ...
        redisRepository.saveRefresh(refreshToken, String.valueOf(member.getMemberId()));
